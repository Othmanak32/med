...
function CustomerPaymentList() {
  ...
  const { data: payments, isLoading, error, refetch } = useQuery(
    ['customer-payments', searchQuery],
    async () => {
      const response = await axios.get('/api/payments/customers', {
  ...
  const deleteMutation = useMutation(
    async (id) => {
      await axios.delete(`/api/payments/customers/${id}`);
    },
    {
      onSuccess: () => {
        refetch();
  ...
  const handlePrint = async () => {
    try {
      const response = await axios.get(
        `/api/payments/customers/${selectedPayment.id}/print`,
        {
          responseType: 'blob',
        }
      );
      ...
      link.setAttribute(
        'download',
        `سند-قبض-${selectedPayment.receipt_number}.pdf`
      );
  ...
  return (
    <>
      <PageHeader
        title="سندات القبض"
        actionText="إضافة سند قبض"
        onActionClick={() => navigate('/payments/customers/new')}
      />
      ...
      >
        <MenuItem
          onClick={() => {
            handleMenuClose();
            navigate(`/payments/customers/${selectedPayment?.id}`);
          }}
        >
      ...
      >
        <DialogTitle>حذف سند القبض</DialogTitle>
        <DialogContent>
          <DialogContentText>
